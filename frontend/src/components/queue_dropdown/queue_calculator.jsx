...
class QueueCalculator extends React.Component {
  ...
  render() {
    const buttonRenders = ["Netflix", "Hulu", "HBO", "AmazonPrimeVideo"].map(
      (provider, idx) => {
        const buttonClass = this.state[provider]
          ? "calculator-btn selected"
          : "calculator-btn";
        const buttonName =
          provider === "AmazonPrimeVideo" ? "Amazon" : provider;
        return (
          <button
            key={idx}
            className={buttonClass}
            onClick={this.handleClick(provider)}
          >
            {buttonName}
          </button>
        );
    ...
    return (
      <div className="calculator flex">
        ...
        <div className="calculator-btns flex">{buttonRenders}</div>
